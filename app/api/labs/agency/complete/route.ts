...
export async function POST() {
  try {
    ...
    const mapSnap = await db.collection("agency_maps").doc(uid).get();
    ...
    const fields = [
      String(data.awarenessPatterns ?? ""),
      String(data.decisionPatterns ?? ""),
      String(data.actionPatterns ?? ""),
    ];
    ...
    const completionRef = db.collection("lab_completion").doc(`${uid}_agency`);
    ...
    return ok({ labKey: "agency", completedAt });
  } catch (error) {
    ...
    console.error("/api/labs/agency/complete POST error:", error);
    return fail("SERVER_ERROR", "Failed to complete agency lab.", 500);
  }
}
