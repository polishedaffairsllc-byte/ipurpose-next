...
export default function IdentityLabPage() {
  ...
  const integrate = async () => {
    setStatus(null);
    try {
      const summary = `Identity Lab snapshot: Self-Perception (${selfPerceptionMap.length} chars), Self-Concept (${selfConceptMap.length} chars), Self-Narrative (${selfNarrativeMap.length} chars)`;
      const res = await fetch("/api/labs/identity/integrate", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          summary,
